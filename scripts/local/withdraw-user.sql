...
--   select id, kakao_id, role, deleted_at from users;
--
-- 제약
--   - 구매자·판매자 행은 거래 상대방 식별에 쓰이므로 남긴다. 배송지와 팔로우 관계만 지운다.
...
from follows
where customer_id in (select id from customers where user_id = @user_id)
   or seller_id in (select id from sellers where user_id = @user_id);

-- 닉네임과 상점명은 unique 라 값을 그대로 두면 그 이름을 아무도 다시 쓸 수 없다.
-- 탈퇴 API 와 같은 규칙(del_{12자리})으로 자리를 비운다.
update customers
set nickname = concat('del_', substring(replace(uuid(), '-', ''), 1, 12))
where user_id = @user_id;

update sellers
set shop_name = concat('del_', substring(replace(uuid(), '-', ''), 1, 12))
where user_id = @user_id;

-- kakao_id 도 unique 라 값을 그대로 두면 같은 계정으로 재가입할 수 없다.
update users
